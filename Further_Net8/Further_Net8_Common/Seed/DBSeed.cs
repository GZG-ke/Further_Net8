...
namespace Further_Net8_Common.Seed
{
    public class DBSeed
    {
        ...
        public static async Task SeedAsync(MyContext myContext, string WebRootPath)
        {
            try
            {
                ...
                var referencedAssemblies = Directory.GetFiles(path, "Further_Net8_Model.dll")
                   .Select(Assembly.LoadFrom).ToArray();
                var modelTypes = referencedAssemblies
                   .SelectMany(a => a.DefinedTypes)
                   .Select(type => type.AsType())
                   .Where(x => x.IsClass && x.Namespace is "Further_Net8_Model.Models")
        ...
        public static void MigrationLogs(MyContext myContext)
        {
            ...
            var modelTypes = referencedAssemblies
               .SelectMany(a => a.DefinedTypes)
               .Select(type => type.AsType())
               .Where(x => x.IsClass && x.Namespace != null && x.Namespace.StartsWith("Further_Net8_Model.Logs"))
               .ToList();
